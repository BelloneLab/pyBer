...
class PostProcessingPanel(QtWidgets.QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.btn_action_undo.setAccessibleName("Undo")
        ...
        self.btn_action_redo.setAccessibleName("Redo")
    ...
    def _export_sync_aligned_files(self) -> None:
        ...
        # Re-export through the shared pyBer v1.0 writer so aligned files follow
        # exactly the same column/sidecar standard as the preprocessing export.
        # The time_aligned column is emitted automatically because each proc
        # carries sync_aligned_time.
        ...
        for proc in ready:
            ...
            stem = os.path.join(out_dir, f"{fid}_time_aligned")
            try:
                if do_csv:
                    export_processed_csv(stem + ".csv", proc, selection=selection)
                if do_h5:
                    export_processed_h5(stem + ".h5", proc, selection=selection)
            except Exception as e:
                self.sync_status.set(f"Aligned export failed for {fid}: {e}", "warn")
                continue
            count += 1
