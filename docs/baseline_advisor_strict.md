# Strict baseline advisor API

This documents the optional `recommend_baseline` API in
`pyBer/baseline_advisor.py`. The application now uses the faster
[inline suggestions](baseline_advisor.md). This strict API retains chronological
holdout checks and abstention for callers requiring that policy; it is not the
method behind the inline percentage fit scores.
